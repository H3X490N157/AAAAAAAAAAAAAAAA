        ...
        with sqlite3.connect('database.db') as con:
                try:
                    ...
                    cur.execute('UPDATE users SET firstName = ?, lastName = ?, address1 = ?,'
                                ...
                                ' phone = ? WHERE email = ?', (firstName, lastName, address1,
                                                               address2, zipcode, city, state, country, phone, email))
...
def cart():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT products.productId, products.name, products.price, products.image FROM products,"
                    " kart WHERE products.productId = kart.productId AND kart.userId = ?", (userId, ))
        ...
        with sqlite3.connect('database.db') as con:
            try:
                ...
                cur.execute('INSERT INTO users (password, email, firstName,'
                            ' lastName, address1, address2, zipcode, city, state, country,'
                            ' phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (hashlib.md5(password.encode()).hexdigest(),
